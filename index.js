  ...
  try {
    ...
    if (user_id) {
      result = await pool.query(
        "SELECT * FROM bookings WHERE user_id = $1 ORDER BY date DESC, time_start",
        [user_id]
      );
    } else {
...
app.post("/api/bookings", async (req, res) => {
  try {
    ...
    // Prevent double booking (check time overlap)
    const conflict = await pool.query(
      `SELECT * FROM bookings
       WHERE court_type = $1 AND court_number = $2 AND date = $3
       AND (time_start < $5 AND time_end > $4)`,
    ...
    const result = await pool.query(
      `INSERT INTO bookings (user_id, court_type, court_number, date, time_start, time_end, description, created_at)
